...
# Reward Function
# -----------------------------
def compute_reward(prev_pos, curr_pos, next_cp, checkpoint_reached=False, finished=False):
...
# RL Agent Skeleton
# -----------------------------
# Action space: up, down, left, right, gas
actions = ["up", "down", "left", "right", "gas", "none"]
...
try:
    while True:
        ...
        # Compute reward
        reward = compute_reward(prev_pos, curr_pos, next_cp, checkpoint_reached, finished)
        prev_pos = curr_pos

        # Select action (placeholder random policy)
        ...
        # Debug
        print(f"Pos: ({x:.2f},{y:.2f},{z:.2f}) | DistCP: {dist_to_cp:.2f} | Action: {action} | Reward: {reward:.2f}")
