...
# ------------------------------------------------------------------#
    ...
    def _parse_hud(self, text: str) -> dict:
        ...
        #Parse a list of all numbers found in the OCR output.
        #The numbers are expected to be in this order (0‑based indices):

        #0‑2   : Position      (x, z, y)
        #3‑5   : Velocity      (vx, vz, vy)
        #6     : Yaw
        #7     : Pitch
        #8     : Roll
        #9     : Gear
    ...
    # --------------------------------------------------------------------- #
# track width should go to this block so the AI knows that the checpoints are not strict coordinates TRACK_WIDTH = 37.0          # Rough width of the track (used only for visualisation)
        ...
        try:
            while True:
                ...
                for act in actions:
                    ...
                    # Every 5th action we read the HUD so that we can
                    # decide whether to stop early (e.g. crash)
                    if len(self.current_episode.actions) % 5 == 0:
